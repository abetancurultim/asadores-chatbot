...
export async function searchProductUrl(sku) {
    const { data, error } = await supabase
        .from("inventory")
        .select("url")
        .eq("sku", sku)
...
export async function saveNickname(nickname, phoneNumber) {
    const { data, error } = await supabase
        .from("users")
...
export async function sendProductImagesToWhatsApp(sku, phoneNumber) {
    console.log("Enviando imágenes del producto con SKU:", sku, "al número:", phoneNumber);
    try {
            ...
            if (typeof imageUrl === "string" && imageUrl.trim() !== "") {
                try {
                    ...
                    if (imageUrl !== compatibleImageUrl) {
                        console.log(`🔄 Convirtiendo formato: ${imageUrl} → ${compatibleImageUrl}`);
                    }
                    // Guardar primero el mensaje en la BD con la URL que se enviará
                    const messageId = await saveChatHistory(phoneNumber, "Archivo enviado", false, compatibleImageUrl);
                    const message = await client.messages.create({
                        to: `whatsapp:${phoneNumber}`,
                        from: "whatsapp:+5742044644",
                        // from: "whatsapp:+14155238886",
        ...
        if (totalSent > 0) {
            ...
            console.log(`📱 Esperando a que lleguen todas las ${totalSent} imagen(es) al usuario... (${finalDelay}ms)`);
        ...
        return JSON.stringify({
            success: totalSent > 0,
            message: totalSent > 0
                ? `Se enviaron ${totalSent} imagen(es) del producto ${sku}${hasFailures ? ` (${failedImages.length} fallaron)` : ""}`
