...
export async function saveChatHistory(clientNumber, newMessage, isClient, newMediaUrl, customSender, origin, fileName) {
    try {
        ...
        if (existingConversation) {
            ...
            console.log(`Using existing conversation ${conversationId} for ${clientNumber}`);
            ...
            if (isClient && existingConversation) {
                ...
                if (!statusError && statusData && statusData.chat_status !== "closed") {
                    const { error: updateError } = await supabase
                        ...
                        .eq("id", conversationId);
                    if (updateError) {
                        console.log(`Error reiniciando flags de notificación para la conversación ${conversationId}:`, updateError.message);
                    }
                    else {
                        console.log(`Flags de notificación reiniciados para la conversación ${conversationId}`);
                    }
                }
                ...
                if (!statusError && statusData && statusData.is_archived === true) {
                    const { error: unarchiveError } = await supabase
                        ...
                        .eq("id", conversationId);
                    if (unarchiveError) {
                        console.log(`Error desarchivando conversación ${conversationId}:`, unarchiveError.message);
                    }
                    else {
                        console.log(`Conversación ${conversationId} desarchivada automáticamente por nuevo mensaje del cliente`);
                    }
                }
                ...
                if (!statusError && statusData && statusData.chat_status === "closed") {
                    try {
                        ...
                        if (!lastMessageError && lastMessage) {
                            ...
                            const timeDifferenceMinutes = (currentTime.getTime() - lastMessageTime.getTime()) /
                                (1000 * 60);
                            ...
                            if (timeDifferenceMinutes >= 5) {
                                const { error: reopenError } = await supabase
                                    ...
                                    .eq("id", conversationId);
                                if (reopenError) {
                                    console.log(`Error reabriendo conversación cerrada ${conversationId}:`, reopenError.message);
                                }
                                else {
                                    console.log(`Conversación ${conversationId} reabierta automáticamente. Han pasado ${Math.floor(timeDifferenceMinutes)} minutos desde el último mensaje.`);
                                }
                            }
                            else {
                                console.log(`Conversación ${conversationId} está cerrada pero solo han pasado ${Math.floor(timeDifferenceMinutes)} minutos desde el último mensaje. Se requieren 5+ minutos para reabrir automáticamente.`);
                            }
                        }
                    }
                    catch (reopenError) {
                        console.log(`Error verificando tiempo para reabrir conversación ${conversationId}:`, reopenError);
                    }
                }
            }
            ...
            if (!isClient && existingConversation) {
                ...
                if (!statusError && statusData && statusData.is_archived === true) {
                    const { error: unarchiveError } = await supabase
                        ...
                        .eq("id", conversationId);
                    if (unarchiveError) {
                        console.log(`Error desarchivando conversación ${conversationId} al enviar mensaje del agente:`, unarchiveError.message);
                    }
                    else {
                        console.log(`Conversación ${conversationId} desarchivada automáticamente al enviar mensaje del agente`);
                    }
                }
            }
        }
            ...
            try {
                ...
                console.log("Info: Could not fetch user data, proceeding with conversation creation");
            }
        ...
        console.log(`Message saved successfully to conversation ${conversationId}, message ID: ${messageId}`);
        return messageId;
    }
    catch (error) {
...
export async function saveTemplateChatHistory(clientNumber, newMessage, isClient, newMediaUrl, user) {
    try {
        ...
        if (existingConversation) {
            ...
            if (!statusError && statusData && statusData.is_archived === true) {
                const { error: unarchiveError } = await supabase
                    ...
                    .eq("id", conversationId);
                if (unarchiveError) {
                    console.log(`Error desarchivando conversación ${conversationId} al enviar plantilla:`, unarchiveError.message);
                }
                else {
                    console.log(`Conversación ${conversationId} desarchivada automáticamente al enviar plantilla`);
                }
            }
        }
        else {
            ...
            try {
                ...
                if (insertError) {
                    ...
                    conversationId = conflictConversation.id;
                    console.log(`Using conversation created by concurrent template request: ${conversationId}`);
                }
                else {
                    conversationId = newConversation.id;
                ...
                throw new Error(`Error creating conversation for template: ${createError}`);
            }
        }
        ...
        const { data: messageData, error: messageError } = await supabase
            ...
            .select("id");
        if (messageError) {
            throw new Error(`Error inserting template message: ${messageError.message}`);
        }
        const messageId = messageData?.[0]?.id;
        console.log(`Template message saved successfully to conversation ${conversationId}, message ID: ${messageId}`);
        return messageId;
    }
    catch (error) {
...
export async function updateMessageTwilioSid(messageId, twilioSid) {
    try {
