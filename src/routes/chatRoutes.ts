...
router.post("/asadores/receive-message", async (req, res) => {
  ...
  try {
    ...
    if (audioUrl) {
      incomingMessageId = await saveChatHistory(
        fromNumber,
        incomingMessage,
        true,
        audioUrl,
    ...
    if (
      ...
      if (responseMessage.length > 1000) {
        ...
        for (let part of messageParts) {
          if (part !== "") {
            const partMessageId = await saveChatHistory(
              fromNumber,
              part,
              false,
              ""
...
router.post("/asadores/chat-dashboard", async (req, res) => {
  try {
    ...
    console.log(
      "📎 Dashboard message - isFileMessage:",
      isFileMessage,
      "fileName:",
    ...
    if (isAudioMessage) {
      ...
      const messageId = await saveChatHistory(
        ...
        userName,
        undefined,
        fileName, // Solo para file messages se pasa el fileName
...
router.post("/asadores/send-template", async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(
      `https://ultim.online/asadores/message/${message.sid}`
    );
    ...
    const messageId = await saveTemplateChatHistory(
      ...
      "",
      user,
      advisorId // Pasar advisorId para asociar plantilla al asesor correcto
    );
...
router.post("/asadores/webhook/status", async (req: express.Request, res: express.Response) => {
    try {
      ...
      const historyData: MessageStatusHistory = {
